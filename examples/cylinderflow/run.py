...
def main():
    ...
    if args.mode == "train":
        if args.custom:
            ...
            train(model, cfg[expt], train_path, meta_path, max_tstep=max_tstep, train_traj_ids=train_traj_ids)
        else:
        ...
        if args.custom:
            ...
            evaluate(model, cfg[expt], train_path, meta_path, dt_step=dt_step, test_traj_ids=test_traj_ids, zeroE=args.zeroE)
        else:
            evaluate(model, cfg[expt], test_path, meta_path, dt_step=dt_step, zeroE=args.zeroE)
