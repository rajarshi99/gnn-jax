...
for plot_info in plot_info_list:
    df = pd.read_csv(plot_info["run_dir"] / train_fname)
    plt.plot(df["step"], df["loss"], alpha=0.8, color=plot_info["color"], label=plot_info["label"])
plt.legend()
plt.grid()
plt.yscale("log")
plt.xlabel("Step")
...
def get_rollout_err(traj_rollout_path):
    ...
    err = jnp.linalg.norm(err, axis=-1)
    err = err.mean(axis=-1)
    return err

for traj_id in range(10):
    for plot_info in plot_info_list:
        err = get_rollout_err(plot_info["run_dir"] / rollout_dirs["On Unseen R"] / f"traj_{traj_id:04d}.npz")
    ...
    plt.ylabel("Error")
    plt.savefig(out_dir / f"rollout_err_{traj_id:04d}.png")
    plt.close()
